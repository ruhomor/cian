...
class CianParser(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        item['jkname'] = response.xpath('//div[@data-name="JkName"]/div/a/text()').extract()  # i dont like this xpath
        item['deadline'] = response.xpath('//div[@data-name="Deadline"]/text()').extract()  # i dont like this xpath
        item['address'] = response.xpath('//div[@data-name="AddressItem"]/span/@content').extract()
        item['metro'] = response.xpath('//div[@data-name="Underground"]/a/div[2]/text()').extract()  # i dont like this xpath
        item['price'] = response.xpath('//div[@data-name="TopPrice"]/div[1]/text()').extract()  # i dont like this xpath
        item['price_pm'] = response.xpath('//div[@data-name="TopPrice"]/div[2]/text()').extract()  # i dont like this xpath
        item['phone'] = response.xpath('//span[@data-name="PhoneText"]/text()').extract()  # i dont like this xpath ? click ?
    ...
    def nextpage(self):
        self.page_num += 1
        return f'https://cian.ru/cat.php?deal_type=sale&engine_version=2&offer_type=flat&p={self.page_num}&region=1&room1=1&room2=1'
